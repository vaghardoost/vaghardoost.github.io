...
export const get = async (id: string) => {
  const { data: noteResponse } = await axios.get<ApiResult<Note>>(`http://localhost:31376/note/${id}`);
  if (noteResponse.success) {
    const note = noteResponse.payload;
    const { data: catResponse } = await axios.get<ApiResult<Category>>(`http://localhost:31376/category/${note.category}`);
